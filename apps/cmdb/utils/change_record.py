...
def create_change_record(
    inst_id, model_id, label, _type, before_data=None, after_data=None, operator=""
):
    """创建实例变更记录"""
    ...
    ChangeRecord.objects.create(
        inst_id=inst_id, model_id=model_id, label=label, type=_type, **change_data
    )
...
def batch_create_change_record(label, _type, change_records, operator=""):
    """创建实例变更记录"""
    batch_change_data = [
        ChangeRecord(label=label, type=_type, operator=operator, **change_record)
