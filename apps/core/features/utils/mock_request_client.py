...
class MockRequestClient:
    def __init__(self):
        ...
        self.admin_token_info: UserTokenEntity = self.client.get_token(
            self.keycloak_test_admin, self.keycloak_test_admin_password
        )

    def get(self, url, role="admin"):
        headers = {"Authorization": f"{self.admin_token_info.token}"}
    ...
    def post(self, url, data, role="admin"):
        ...
        response = requests.post(
            f"{self.test_base_url}/{url}", json=data, headers=headers
        )
        response.raise_for_status()
        return response.json()
